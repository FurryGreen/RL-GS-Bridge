...
# def PILtoTorch(pil_image, resolution):
#     # 使用 Pillow-SIMD 代替标准 Pillow
#     pil_image = pil_image.resize(resolution, resample=Image.BILINEAR)
#     transform = transforms.ToTensor()
#     resized_image = transform(pil_image)
    
#     if resized_image.shape[0] == 3:
#         return resized_image
#     else:
#         return resized_image.unsqueeze(0)
def cv2ToTorch(cv2_image, resolution):
    # 调整图像大小
    resized_image = cv2.resize(cv2_image, resolution)
    
    # 转换为 PyTorch 张量并归一化
    resized_image = torch.from_numpy(resized_image).float() / 255.0
