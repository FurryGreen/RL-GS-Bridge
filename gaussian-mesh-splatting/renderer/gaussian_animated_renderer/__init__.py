...
def render(idxs, triangles, viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier=1.0,
           override_color=None, norm_aug = False):
    """
    Render the scene.
    ...
    #print(viewpoint_camera.image_width)
    ...
    if norm_aug:
        _xyz = torch.matmul(
                torch.concat((pc.alpha, pc.soft_normalpha), dim=-1),       #[N,K,3]]
                torch.concat((pc.triangles, pc.v0.unsqueeze(1)), dim=-2)
            )
    else:
        _xyz = torch.matmul(
            pc.alpha,
