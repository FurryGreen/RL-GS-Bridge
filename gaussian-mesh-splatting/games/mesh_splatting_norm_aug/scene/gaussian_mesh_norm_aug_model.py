...
class GaussianMeshNormAugModel(GaussianModel):
    ...
    def __init__(self, sh_degree: int):
        ...
        self.soft_normalpha = torch.empty(0)
        self.v0 = torch.empty(0)
    ...
    def create_from_pcd(self, pcd: MeshPointCloud, spatial_lr_scale: float):
        ...
        soft_normalpha = torch.zeros((alpha_point_cloud.shape[0],alpha_point_cloud.shape[1],1), dtype=torch.float, device="cuda")
        self._soft_normalpha = nn.Parameter(soft_normalpha.requires_grad_(True))
        #print("leaf:", self._soft_normalpha.is_leaf)
        self.prepare_v0()
        self.update_alpha()
    ...
    def prepare_v0(self):
        ...
        v0 = normals / (torch.linalg.vector_norm(normals, dim=-1, keepdim=True) + 1e-8)
        self.v0 = v0#.cuda()
    ...
    def _calc_xyz(self):
        ...
        """
        #print("v0 shape:", self.v0)
        _xyz = torch.matmul(
            torch.concat((self.alpha, self.soft_normalpha), dim=-1),       #[N,K,3]]
            torch.concat((self.triangles, self.v0.unsqueeze(1).cuda()), dim=-2)
        )
    ...
    def update_alpha(self):
        ...
        self.soft_normalpha =  torch.tanh(self._soft_normalpha)
        #print("alpha:", self.alpha.is_leaf)
        self._calc_xyz()
    ...
    def training_setup(self, training_args):
        ...
        l_params = [
            ...
            {'params': [self._soft_normalpha], 'lr': training_args.alpha_lr, "name": "soft_alpha"}
        ]
        #print(self.vertices.is_leaf)
    ...
    def load_ply(self, path):
        ...
        alpha = params['_alpha']
        scale = params['_scale']
        soft_normalpha = params['_soft_normalpha']
        ...
        self._soft_normalpha = nn.Parameter(soft_normalpha)
